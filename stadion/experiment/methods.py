...
from definitions import BASELINE_GIES, BASELINE_DCDI, BASELINE_IGSP, BASELINE_LLC, BASELINE_NODAGS, \
    BASELINE_KDS, BASELINE_OURS, FOLDER_TRAIN, FOLDER_TEST

from baselines import run_igsp, run_gies, run_dcdi, run_llc, run_nodags

from stadion.run import run_algo_benchmark as run_stationary_diffusion
...
if __name__ == "__main__":
    ...
    if base_method == BASELINE_GIES:
    ...
    elif BASELINE_KDS in base_method or BASELINE_OURS in base_method:
        
        key, subk = random.split(random.PRNGKey(kwargs.seed))
        
        # run ours
        with timer() as walltime:
            sampler, intv_theta_initializer, theta, intv_param_train = \
                run_stationary_diffusion(key, train_targets, test_intv, config=config, eval_mode=True)
        ...
        intv_param_test = InterventionParameters(parameters=intv_theta_test, targets=test_intv)
        
        # sample train predictions
        key, subk = random.split(key)
        pred_samples_train = sampler(subk, kwargs.n_samples, intv_param=intv_param_train)

        # sample test predictions
        key, subk = random.split(key)
        pred_samples_test = sampler(subk, kwargs.n_samples, intv_param=intv_param_test)
        ...
        pred = dict(
            ...
            theta=theta,
            intv_theta_train=intv_param_train._store,
            intv_theta_test=intv_param_test._store,
        )
